export const fetchHabitos = async (token: string) => {
    const response = await fetch("https://backend-swart-eight-45.vercel.app/api/habitos", 
        {
            method: "GET",
            headers: {
...
export const marcarHabitoCompletado = async (habitoId: string, token: string) => {
    const response = await fetch(`https://backend-swart-eight-45.vercel.app/api/habitos/marcarcompletado/${habitoId}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
